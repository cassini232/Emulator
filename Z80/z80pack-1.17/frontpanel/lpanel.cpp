  ...
  if(!strcmp(token, "color"))		// color
  ...
  else if(!strcmp(token, "line"))
   {
	 ...
	 bailout = 1;
       ...
       {
	if(!(curr_element = curr_object->addElement()))
   ...
   {  int n;
	...
	if(n < 2)
         { printf("Error on line %d of config file %s\n",lineno, fname);
           printf("normal must have 2 or 3 coordinates.\n");
	   bailout = 1;
	 }	
